...
std::vector<Film> loadFilms() {
    ...
    io::CSVReader<10, io::trim_chars<' '>, io::no_quote_escape<';'>> in(
        "../movies.csv");
    in.read_header(io::ignore_missing_column, "ID", "Title", "Genres", "Year",
                   "Runtime", "Description", "Cast", "Director", "Writers",
                   "Producers");
    unsigned int ID;
    std::string Title;
    std::string Genres;
    ...
    while (in.read_row(ID, Title, Genres, Year, Runtime, Description, Cast,
        ...
        Film f(ID, Title, Year, Genres, Cast, Description, {}, Runtime, Writers,
               Producers, Director);
        list.push_back(f);
    }
    return list;
