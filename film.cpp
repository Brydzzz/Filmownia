...
void Film::addReview(Review review) {
    auto it =
        std::find_if(reviews.begin(), reviews.end(),
                     [&review](const Review &existingReview) {
                         return existingReview.getUser() == review.getUser();
                     });
...
void Film::deleteReview(Review review) {
