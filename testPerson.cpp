...
TEST(director_test, find_films_title_test) {
    Director d(3, "Yorgos Lanthimos", 23, 9, 1973);
    Film f1(3, "Poor Things 2", 2018, {film_genre::Drama}, {},
            "England, early 18th century. The close relationship between Queen "
            "Anne and Sarah Churchill is threatened by the arrival of Sarah's "
            "cousin, Abigail Hill, resulting in a bitter rivalry between the "
            "two cousins to be the Queen's favourite.",
    ...
    d.addFilm(f1);
    d.addFilm(f2);
    std::vector<Film*> found = d.findFilmsByTitle("Poor Things");
    ASSERT_EQ(found.size(), 2);
}

TEST(director_test, find_films_title_not_found_test) {
    Director d(3, "Yorgos Lanthimos", 23, 9, 1973);
    Film f1(3, "Poor Things 2", 2018, {film_genre::Drama}, {},
            "England, early 18th century. The close relationship between Queen "
            "Anne and Sarah Churchill is threatened by the arrival of Sarah's "
    ...
    d.addFilm(f1);
    d.addFilm(f2);
    std::vector<Film*> found = d.findFilmsByTitle("Not Here");
...
TEST(writer_test, display_info) {
    ...
    w.addJob(WriterType::Writer, f2);
    std::stringstream ss;
    w.displayWriterInfo(ss);
    ASSERT_EQ(ss.str(),
              "Jon Spaihts\nBirthdate: 1970-02-04\nSelected films: \n\"Dune\" "
              "(2021)\n\"Passengers\" (2016)\n");
}

TEST(writer_test, display_info_no_jobs) {
    Writer w(5, "Jon Spaihts", 4, 2, 1970);
    std::stringstream ss;
    w.displayWriterInfo(ss);
    ASSERT_EQ(ss.str(), "Jon Spaihts\nBirthdate: 1970-02-04\n");
}
